...
manifest=json.loads(Path('D:/Agent Workspaces/Agent Delivery/BA_Animation_Workflow/releases/0.14.0/build-manifest.json').read_text())
assert tomllib.loads((base/'blender_manifest.toml').read_text(encoding='utf8'))['version']=='0.14.0'
...
assert tuple(m.constants.ADDON_VERSION)==(0,14,0)
...
m.register()
out=Path('D:/Agent Workspaces/Agent Temp/BA_Animation_Workflow/testing/optimization/installed_report.json')
report={'version':'0.14.0','installed_path':str(base),'enabled':True,'files_match_build':True,'operators_present':True,'actual_installed_analyze_and_preview':True,'unregister_restores_preview':True,'register_cycle':True}
