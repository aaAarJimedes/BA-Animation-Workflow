...
适用版本：Blender 5.1.2、Proscenium Motion Bridge 0.9.1、BA Animation Workflow 0.7.0、Proscenium 0.4.0、MMD Tools 4.5.13。BlendCap 与 BlendCap Motion Bridge 仅属于另一条传统 BVH 工作流，不是本插件依赖。
...
既有真实星野 MMD 骨架回归结果为 24/24、世界旋转误差 0°、6 个精确约束快照和 0 个临时 Action 残留。0.9.1 在此基础上增加跟随目标初始布置的动作空间，以及强度可调的肢端防穿模；发布前仍应在角色工程副本中用实际动作目测脚底、双手近距离动作和端枪姿态。
